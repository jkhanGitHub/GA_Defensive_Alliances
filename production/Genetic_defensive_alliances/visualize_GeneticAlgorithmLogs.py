...
def visualize_ga_results(csv_file, worst_possible_fitness=None):
    ...
    if "Worst Possible Fitness" in config:
        try:
            worst_possible_fitness = float(config["Worst Possible Fitness"])
        except ValueError:
            pass
    ...
    ax0.text(0.02, 0.5, config_text, fontsize=9, family='monospace',
    ...
    if 'python' in sys.argv[0] and 'visualize_ga.py' in sys.argv[0]:
        plt.show()
